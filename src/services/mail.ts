...
export const MailService = {
  async sendLoginCode(
    ...
    await transport.sendMail({
      from: `"${process.env.MAIL_FROM_NAME || 'FFM Owianzeiger'}" <${process.env.MAIL_FROM}>`,
      to: email,
      ...
      html: [
        ...
        '<p>Mit freundlichen Grüßen<br>FFM Owianzeiger</p>',
      ].join('\n'),
    })
  },
  ...
  async sendReport(
    ...
    await transport.sendMail({
      ...
      cc: user.email,
      subject: `Anzeige Ordnungswidrigkeit – Kfz ${report.kennzeichen}`,
      text: [
